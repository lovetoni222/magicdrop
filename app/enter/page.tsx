...
import { useEffect, useRef, useState } from "react"; import { motion, AnimatePresence } from "framer-motion"; import { Sparkles, Mail, Users, Star, X, RotateCcw } from "lucide-react"; import "../globals.css";
...
const [state, setState] = useState(() => stickers.reduce((acc, s) => { acc[s.id] = { x: Math.floor(Math.random() * 200) + 60, y: Math.floor(Math.random() * 250) + 100, scale: 1, rotation: 0, }; return acc; }, {} as Record<string, { x: number; y: number; scale: number; rotation: number }>) );
